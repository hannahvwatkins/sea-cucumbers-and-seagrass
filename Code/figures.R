#This script is for the figures in Watkins et al., 2023 in Ecosphere
...
source("Code/ggplot_paper_theme.R") #custom theme script
...
init <- read_csv("Data/main_sampling_clean.csv") %>% 
  ...
  #relevel the treatment so "natural" is our baseline
  mutate(treatment = factor(treatment, 
                            levels = c("natural", "half", "zero"))) %>% 
  #remove any rows with NAs in the responses
...
init_resp <- init %>% 
  ...
  drop_na()
#condense these variables into principal components
...
fig1a <- ggplot(pca_points, aes(x = PC1, y = PC2)) +
  ...
  theme(axis.title.x = element_text(margin = margin(t = 10, r = 0, 
        ...
        axis.title.y = element_text(margin = margin(t = 0, r = -70, 
                                                    b = 0, l = 0),
                                    size = 10),
...
init_mod_labs <- c("b_scale_sand"="Sand depth",
                   "b_scale_seagrass_area"="Total patch size",
...
fig1b <- mod_init %>% 
  ...
  mutate(`.variable` = factor(`.variable`, 
                              levels = c("b_scale_sand",
                                         ...
                                         "b_scale_log_og_cukes:scale_log_grunt_pee_m2",
                                         "b_scale_seagrass_area",
                                         "b_scale_log_og_cukes",
                                         "b_scale_log_distance"))) %>%
  #now plot!
  #colour the posteriors by whether they're above/below 0
  ggplot(aes(y = .variable, x = .value, fill = stat(x < 0))) +
  #reduce size of dot and whiskers since that's only there for the frequentists
  stat_halfeye(.width = 0.95, interval_size_range = c(0.15,0.15)) +
...
figS2b <- ggplot(pca_points2, aes(x = PC1, y = PC2)) +
  ...
  geom_segment(data = pca_load2, 
               aes(x = 0, y = 0, 
                   #scale segment ends to their default autoplot values
                   xend = PC1*5,
                   yend = PC2*5),
               arrow = arrow(length = unit(1/4, 'picas')),
               size = 0.2) +
  annotate('text', x = (pca_load2$PC1*8), y = (pca_load2$PC2*5.2),
...
#FIGURE S3-------------------------------------------------------
#create nice labels
main_mod_labs <- c("b_scale_seagrass_area" = "Total patch size",
                   ...
                   "b_scale_log_distance"="log(Distance from reef)")
main_mod_labs_treatment <- c("b_scale_seagrass_area" = "Total patch size",
...
figS3a <- mod_main_density %>% 
  ...
  filter(`.variable` != "b_Intercept") %>% 
  mutate(`.variable` = factor(`.variable`, 
                              levels = c("b_scale_seagrass_area",
...
#a sequence across its entire range)
newdata <- tibble(scale_log_cuke_density_final = rep(seq(from = -1.2, to = 1.74, 
                                                         by = 0.01), times = 3),
                  #we'll use the mean and the mean +/- 1SD to get a good range
                  scale_log_grunt_pee_m2 = c(rep(0, times = 295), 
                                             rep(0, times = 295),
                                             rep(0, times = 295)),
                  scale_distance = rep(0),
                  scale_below_per_shoot = rep(0),
                  scale_seagrass_area = rep(0)) 
...
#ggsave("Figures/Figure2a.png", fig2a, device = "png",
#       height = 100, width = 90, units = "mm", dpi = 600)
...
fig2b <- mod_growth %>% 
  ...
  theme(legend.position = "none",
        ...
        #create same margin as panel A
        axis.title.x = 
          element_text(margin = margin(t = 2, r = 0, b = 0, l = 0), size = 10),
        plot.title = element_text(hjust = -0.5, vjust = 2.5, size = 12)) +
  labs(x = "Coefficient estimate", y = "",
       title = "B")
fig2b
...
#       height = 100, width = 180, units = "mm", dpi = 600)
#ggsave("Figures/Figure2.pdf", fig2, device = "pdf",
#       height = 100, width = 180, units = "mm", dpi = 600)
